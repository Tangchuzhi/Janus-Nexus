...
(function() {
    ...
    async function cleanupAssistantCharacters() {
        try {
            debugLog('开始清理多余的assistant角色卡和对话框...');
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // 查找所有名为"assistant"的角色
                    assistantCharacters = characters.filter(char => 
                        char.name && char.name.toLowerCase() === 'assistant'
                    );
                    
                    debugLog(`找到 ${assistantCharacters.length} 个assistant角色`);
                }
            } catch (apiError) {
                debugLog('无法从API获取角色列表，尝试其他方法: ' + apiError.message);
                
                // 备用方法：从全局对象获取
                if (window.characters && Array.isArray(window.characters)) {
                    assistantCharacters = window.characters.filter(char => 
                        char.name && char.name.toLowerCase() === 'assistant'
                    );
                    debugLog(`从全局对象找到 ${assistantCharacters.length} 个assistant角色`);
                }
            }
            
            // 3. 智能清理assistant角色和对话框
            if (assistantCharacters.length > 0) {
                debugLog(`发现 ${assistantCharacters.length} 个assistant角色`);
                
                // 检查每个assistant角色是否有对话内容
                for (const char of assistantCharacters) {
                    try {
                        // 尝试获取该角色的对话历史
                        const chatResponse = await fetch('/api/chat/get', {
                            method: 'POST',
                            headers: context.getRequestHeaders(),
                            body: JSON.stringify({ 
                                character: char.name,
                        ...
                        if (chatResponse.ok) {
                            ...
                            const hasUserContent = messages.some(msg => 
                                ...
                                !msg.mes.includes('System:') && 
                                !msg.mes.includes('Assistant:')
                            );
                            
                            if (!hasUserContent) {
                                debugLog(`assistant角色 ${char.name} 没有用户内容，将删除`);
                                cleanupCommands += `/sendas ${char.name} ||\n`;
                                cleanupCommands += `/delete ||\n`;
                            } else {
                                debugLog(`assistant角色 ${char.name} 有用户内容，保留`);
                            }
                        } else {
                            // 如果无法获取对话数据，且是多余的assistant角色，则删除
                            if (assistantCharacters.length > 1) {
                                debugLog(`无法获取assistant角色 ${char.name} 的对话数据，将删除`);
                                cleanupCommands += `/sendas ${char.name} ||\n`;
                                cleanupCommands += `/delete ||\n`;
                            }
                        }
                    } catch (chatError) {
                        debugLog(`检查assistant角色 ${char.name} 对话时出错: ${chatError.message}`);
                        // 如果出错且是多余的assistant角色，则删除
                        if (assistantCharacters.length > 1) {
                            cleanupCommands += `/sendas ${char.name} ||\n`;
                            cleanupCommands += `/delete ||\n`;
                        }
            ...
            // 5. 执行清理命令
            if (cleanupCommands.includes('/sendas') || cleanupCommands.includes('/delete')) {
                debugLog('执行清理命令序列:');
                debugLog(cleanupCommands);
